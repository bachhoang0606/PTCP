...
void compileConstant2(void) {
  ...
  default:error(ERR_INVALIDSTATEMENT, lookAhead->lineNo, lookAhead->colNo);
    break;
  }
}
...
void compileParams2(void) {
  ...
  if (lookAhead->tokenType == SB_SEMICOLON) {
    eat(SB_SEMICOLON); compileParam(); compileParams2();
  }
}
...
void compileExpression(void) {
  ...
  // TODO
  switch (lookAhead->tokenType) {
  case SB_PLUS: eat(SB_PLUS);  break;
  case SB_MINUS: eat(SB_MINUS); break; 
  }
  compileExpression2();
...
void compileFactor(void) {
  // TODO
  switch(lookAhead->tokenType) {
  case TK_NUMBER: eat(TK_NUMBER); break;  // const
  case TK_CHAR: eat(TK_CHAR); break;      // const
  case TK_IDENT:
    eat(TK_IDENT);
    switch (lookAhead->tokenType)  {
      case SB_LSEL: compileIndexes(); break; // valriale
      case SB_LPAR: compileArguments(); // compileFunctionApplication
      default: ; break; //UnsignedConstant Char
    } 
    break;    // variable or function application
  case SB_LPAR: eat(SB_LPAR); compileExpression(); eat(SB_RPAR); break;
  default: error(ERR_INVALIDFACTOR, lookAhead->lineNo, lookAhead->colNo); break;
  }
}
